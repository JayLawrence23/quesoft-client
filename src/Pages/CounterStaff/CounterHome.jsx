import { AppBar, Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, Paper, Badge, Slide, Tab, Tabs, Typography } from '@material-ui/core';
...
import { callCustomer, ticketOnCounterStaff, 
    queuingComplete, missedCustomer, 
    countWaitingByService, countMissedByService,
    countServedByService } from '../../Actions/transaction';
import { showMissed, searchMissedByService,
    serveMissedTicket, completeMissedTicket } from '../../Actions/counterStaff';
...
const CounterHome = () => {
    ...
    const  { countWaiting, countServed, countMissed } = useSelector((state) => state.count);
    ...
    const handleSearchMissed = (e) => {
        ...
        if(validate()){
            dispatch(searchMissedByService(values, setOpenDialog, setIsMissedTicket, setIsValid))
        }
    }
    ...
    return ( 
        <CounterStaffLayout>
            <Paper className={classes.paperheader}>
                ...
                <Typography variant="h5" style={{ fontWeight: 'bold'}}>{valuescounter.currentService} </Typography>
            </Paper>
                ...
                <TabPanel value={tabValue} index={0}>
                    <Grid container spacing={1}>
                        <Grid item xs={12} sm={8}>
                            { ticket ? 
                                <Ticket ticket={ticket} handleClickOpenDone={handleClickOpenDone} handleClickOpenMissed={handleClickOpenMissed}/>
                            : 
                            (
                                <div>
                                    <Button variant="contained" color="primary" onClick={() => handleCallCustomer(valuescounter)}>Call Customer</Button>
                                </div>
                            ) 
                            }
                        </Grid>
                        <Grid item xs={12} sm={4}>
                            ...
                            <Paper elevation={0} className={classes.rightpaper}>
                                { ticket ? null : 
                                <div>
                                    <Typography variant="subtitle1" component={'span'}>Missed Queue? Type here </Typography>
                            
                                    <Paper elevation={0} style={{ maxWidth: 400}}> 

                                    { isValid && (
                                        <AlertMessage severity="error" message="Ticket Not Found."/>)
                                    }
                                            
                                        <Typography component={'span'} variant="h6">Enter the missed ticket number:</Typography>
                                ...
                                <div className={classes.waiting}>
                                    <Typography variant="subtitle1" component={'span'}>Waiting: <Box style={{ fontWeight: 'bold', color: "#EFB701" }}>{countWaiting}</Box> </Typography>
                                </div>
                                <div className={classes.waiting}>
                                    <Typography variant="subtitle1" component={'span'}>Served: <Box style={{ fontWeight: 'bold',  color: "orange" }}>{countServed}</Box></Typography>
                                </div>
                                <div className={classes.waiting}>
                                    <Typography variant="subtitle1" component={'span'}>Missed: <Box style={{ fontWeight: 'bold', color: "#303443" }}>{countMissed}</Box></Typography>
                                </div>
                            </Paper>
                        </Grid>
                    </Grid>
            ...
            >
                ...
                <DialogContentText id="alert-dialog-slide-description">
                    { isMissedOpen ? "Once you click missed, the customer will miss the queue and will call next customer. Are you sure?" 
                    : isMissedTicket ? "Ticket found. Once you click serve, you'll be serving the customer who missed their queue." : 
                ...
                <DialogActions>
                <Button onClick={ isMissedOpen? handleMissed : isMissedTicket ? handleServeMissed : handleNext} color="primary">
